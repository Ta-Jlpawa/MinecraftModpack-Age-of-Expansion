...
另请参见[桶填充器](bucket-filler.md)。

注意，由于它使用了<ItemLink id="pattern_provider" />，因此该装置应当集成到你的[自动合成](../ae2-mechanics/autocrafting.md)
系统中使用。

有时候事情就是这么不方便：你需要的是流体本身，却只能用桶来产出这种流体。有时某些机器可以帮你完成这件事（例如热力膨胀的流体装填机），但你未必总能找到能方便地做到这一点的模组。幸运的是，
...
<BoxAnnotation color="#dddddd" min="2 1 0" max="3 2 1">
        (1) 样板供应器：设置为“有红石信号时”锁定合成，并开启阻挡模式，放入相关的处理样板。
...
* <ItemLink id="pattern_provider" /> (1) 设置为“有红石信号时”锁定合成，并开启阻挡模式，
  并放入相关的<ItemLink id="processing_pattern" />。
  
    ![充能样板](../assets/diagrams/water_empty_pattern.png)
    ![充能样板](../assets/diagrams/lava_empty_pattern.png)

* <ItemLink id="interface" /> (2) 保持默认配置。
* 第一个<ItemLink id="storage_bus" /> (3) 保持默认配置。
* <ItemLink id="annihilation_plane" /> (4) 没有可配置的界面。
* <ItemLink id="import_bus" /> (5) 过滤为桶。
  <ItemImage id="minecraft:bucket" scale="2" />
...
1. <ItemLink id="pattern_provider" /> 将原料推入<ItemLink id="interface" />。
   （实际上，作为一种优化，它会直接穿过存储总线推送，就像总线是供应器各面的延伸一样。物品实际上从不进入接口内部。）
2. 通过[管道子网络](pipe-subnet.md#providing-to-multiple-places)中描述的机制，
   桶最终进入<ItemLink id="minecraft:dispenser" />中。
3. <ItemLink id="minecraft:comparator" /> 检测到发射器中的桶，从而同时为发射器充能并锁定
   <ItemLink id="pattern_provider" />。
4. 发射器将桶中的流体倒出，此时其内部只剩一个空桶。
5. <ItemLink id="import_bus" /> 将空桶从发射器中抽出，并通过
